...
Altman Z-Score, Beneish M-Score는 의도적으로 제외했습니다 — 둘 다 여러 재무비율을 특정 계수로 합산하는 복합 모델이라, 계수 하나하나의 근거를 설명하기보다는 공시된 수치에서 바로 계산되는 지표(부채비율, 영업이익률)와 규칙 기반 Red Flag에 집중하는 쪽을 택했습니다. 현재 가중치는 부채비율(40%) + 영업이익률(30%) + 스코어 추이(15%) + Red Flag 수(15%) 입니다.
